      ...
      if (node->mesh) {
        for (Primitive *primitive : node->mesh->primitives) {
          for (uint32_t i = 0; i < primitive->vertexCount; i++) {
            Vertex &vertex =
                this->loaderInfo.vertexBuffer[primitive->firstVertex + i];
            // Pre-transform vertex positions by node-hierarchy
            if (preTransform) {
              vertex.pos = localMatrix * glm::vec4(vertex.pos.x, vertex.pos.y,
                                                   vertex.pos.z, 1.0f);
              vertex.normal = glm::normalize(
                  localMatrix * glm::vec4(vertex.normal.x, vertex.normal.y,
...
void Model::updateAnimation(uint32_t index, float time) {
  ...
  for (auto &channel : animations[index].channels) {
    gtp::AnimationSampler &sampler =
        animations[index].samplers[channel.samplerIndex];
    if (sampler.inputs.size() > sampler.outputsVec4.size()) {
      continue;
    }

    for (size_t i = 0; i < sampler.inputs.size() - 1; i++) {
      if ((animations[index].currentTime >= sampler.inputs[i]) &&
          (animations[index].currentTime <= sampler.inputs[i + 1])) {
        float u =
            std::max(0.0f, animations[index].currentTime - sampler.inputs[i]) /
