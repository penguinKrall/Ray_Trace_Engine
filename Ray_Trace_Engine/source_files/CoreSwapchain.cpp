...
void CoreSwapchain::retrieveDeviceSwapchainData(VkPhysicalDevice physicalDevice,
                                                VkSurfaceKHR surface) {

  // get surface capabilities
  vkGetPhysicalDeviceSurfaceCapabilitiesKHR(physicalDevice, surface,
                                            &swapchainData.surfaceCapabilities);

  // output surface capabilities
  gtp::Utilities_EngCore::outputSurfaceCapabilities(
      swapchainData.surfaceCapabilities);
  ...
  if (formatCount != 0) {
    ...
    vkGetPhysicalDeviceSurfaceFormatsKHR(
        ...
        swapchainData.availableSurfaceFormats.data());
  }
  ...
  for (const auto &format : swapchainData.availableSurfaceFormats) {
    std::cout << "\tFormat: "
              << EnumStringHelper::vkFormatToString(format.format)
              << ", Color Space: "
              << EnumStringHelper::ColorSpaceToString(format.colorSpace)
              << "\n";
  }
  ...
  if (presentationCount != 0) {
    ...
    vkGetPhysicalDeviceSurfacePresentModesKHR(
        physicalDevice, surface, &presentationCount,
        swapchainData.availablePresentationModes.data());
  }
  ...
  for (const auto &format : swapchainData.availableSurfaceFormats) {
    if (format.format == VK_FORMAT_R8G8B8A8_UNORM &&
      ...
      std::cout << "\nAssigned Swapchain Image Format: "
                << EnumStringHelper::vkFormatToString(
                       swapchainData.assignedSwapchainImageFormat.format)
                << "\nAssigned Swapchain Image Color Space: "
                << EnumStringHelper::ColorSpaceToString(
                       swapchainData.assignedSwapchainImageFormat.colorSpace)
                << std::endl;
      return;
    }
  }
...
void CoreSwapchain::assignSwapchainPresentMode() {
  for (const auto &presentationMode :
       swapchainData.availablePresentationModes) {
    if (presentationMode == VK_PRESENT_MODE_MAILBOX_KHR) {
      swapchainData.assignedPresentMode = presentationMode;
      std::cout << "\nAssigned Present Mode: "
                << EnumStringHelper::VkPresentModeToString(
                       swapchainData.assignedPresentMode)
                << std::endl;
    }
  }
}
...
void CoreSwapchain::retrieveSurfaceExtent(GLFWwindow *window) {
  ...
  if (swapchainData.surfaceCapabilities.currentExtent.width !=
      std::numeric_limits<uint32_t>::max()) {

    std::cout << std::endl
              << "Swapchain Surface Extent\n"
              << " width: "
              << swapchainData.surfaceCapabilities.currentExtent.width
              << "\n height: "
              << swapchainData.surfaceCapabilities.currentExtent.height
              << std::endl;

    swapchainData.swapchainExtent2D =
        swapchainData.surfaceCapabilities.currentExtent;
    ...
    newExtent.width = std::max(
        swapchainData.surfaceCapabilities.minImageExtent.width,
        std::min(swapchainData.surfaceCapabilities.maxImageExtent.width,
                 newExtent.width));
    newExtent.height = std::max(
        swapchainData.surfaceCapabilities.minImageExtent.height,
        std::min(swapchainData.surfaceCapabilities.maxImageExtent.height,
                 newExtent.height));
...
std::vector<VkImage>
CoreSwapchain::createSwapchainImage(VkDevice logicalDevice) {
  ...
  vkGetSwapchainImagesKHR(logicalDevice, swapchainData.swapchainKHR,
                          &swapchainImageCount, nullptr);

  vkGetSwapchainImagesKHR(logicalDevice, swapchainData.swapchainKHR,
                          &swapchainImageCount,
                          swapchainData.swapchainImages.image.data());
...
CoreSwapchain::createSwapchainImageView(VkDevice logicalDevice) {

  // resize
  swapchainData.swapchainImages.imageView.resize(
      swapchainData.swapchainImages.image.size());
  ...
  for (int i = 0; i < frame_draws; i++) {
    ...
    // Create image view
    if (vkCreateImageView(logicalDevice, &viewCreateInfo, nullptr,
                          &swapchainData.swapchainImages.imageView[i]) !=
        VK_SUCCESS) {
...
// -- create swapchain
VkSwapchainKHR CoreSwapchain::createSwapchain(
  ...
  // Find how many images are in the swap chain. Get one more than the minimum
  // to allow triple buffering.
  ...
  swapchainCreateInfo.preTransform =
      swapchainData.surfaceCapabilities.currentTransform;
  swapchainCreateInfo.compositeAlpha = VK_COMPOSITE_ALPHA_OPAQUE_BIT_KHR;
  swapchainCreateInfo.clipped = false;
  swapchainCreateInfo.imageSharingMode = VK_SHARING_MODE_CONCURRENT;

  std::cout << "graphics queue: " << queueFamilyIndices.graphics
            << "	present queue: " << queueFamilyIndices.present
            << std::endl;
  if (queueFamilyIndices.graphics != queueFamilyIndices.present) {
    std::array<uint32_t, 4> indices = {(uint32_t)queueFamilyIndices.graphics,
    ...
    std::cout << "[1]swapchainCreateInfo.queueFamilyIndexCount = "
              << swapchainCreateInfo.queueFamilyIndexCount << std::endl;
  }
    ...
    std::cout << "[2]swapchainCreateInfo.queueFamilyIndexCount = "
              << swapchainCreateInfo.queueFamilyIndexCount << std::endl;
  }
  ...
  VkResult result = vkCreateSwapchainKHR(logicalDevice, &swapchainCreateInfo,
                                         nullptr, &swapchainData.swapchainKHR);
