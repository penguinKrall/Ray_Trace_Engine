...
void gltf_viewer::Init_gltf_viewer(EngineCore* coreBase) {
	...
	std::cout << "\nfinished creating bottom level acceleration structures." << std::endl;
	...
	std::cout << "\nfinished creating top level acceleration structure" << std::endl;
...
//
//std::cout << "this->assets.gondola->textures.size(): " << this->assets.gondola->textures.size() << std::endl;

}

void gltf_viewer::CreateBottomLevelAccelerationStructures() {
...
void gltf_viewer::CreateTLAS() {
	...
	if (pEngineCore->CreateBuffer(
		...
		throw std::invalid_argument("failed to create gltf_viewer instances buffer");
	}
...
void gltf_viewer::CreateUniformBuffer() {
...
void gltf_viewer::UpdateUniformBuffer(float deltaTime) {
...
void gltf_viewer::CreateRayTracingPipeline() {
	...
	{
		shaderStages.push_back(shader.loadShader(projDirectory.string() + "/shaders/compiled/gltf_viewer_raygen.rgen.spv", VK_SHADER_STAGE_RAYGEN_BIT_KHR,
			"gltf_viewer_raygen"));
	...
	// Miss group
	{
		shaderStages.push_back(shader.loadShader(projDirectory.string() + "/shaders/compiled/gltf_viewer_miss.rmiss.spv", VK_SHADER_STAGE_MISS_BIT_KHR,
			"gltf_viewer_miss"));
		VkRayTracingShaderGroupCreateInfoKHR shaderGroup{};
		shaderGroup.sType = VK_STRUCTURE_TYPE_RAY_TRACING_SHADER_GROUP_CREATE_INFO_KHR;
		shaderGroup.type = VK_RAY_TRACING_SHADER_GROUP_TYPE_GENERAL_KHR;
		...
		shaderStages.push_back(shader.loadShader(projDirectory.string() + "/shaders/compiled/gltf_viewer_shadow.rmiss.spv", VK_SHADER_STAGE_MISS_BIT_KHR,
			"gltf_viewer_shadowmiss"));
		shaderGroup.generalShader = static_cast<uint32_t>(shaderStages.size()) - 1;
		shaderGroups.push_back(shaderGroup);
	}
	...
	{
		shaderStages.push_back(shader.loadShader(projDirectory.string() + "/shaders/compiled/gltf_viewer_closesthit.rchit.spv", VK_SHADER_STAGE_CLOSEST_HIT_BIT_KHR,
			"gltf_viewer_closestHit"));
		VkRayTracingShaderGroupCreateInfoKHR shaderGroup{};
		shaderGroup.sType = VK_STRUCTURE_TYPE_RAY_TRACING_SHADER_GROUP_CREATE_INFO_KHR;
		...
		shaderStages.push_back(shader.loadShader(projDirectory.string() + "/shaders/compiled/gltf_viewer_anyhit.rahit.spv", VK_SHADER_STAGE_ANY_HIT_BIT_KHR,
			"gltf_viewer_anyhit"));
...
void gltf_viewer::CreateDescriptorSet() {
	...
	VkDescriptorImageInfo glassTextureDescriptor{ this->assets.coloredGlassTexture.sampler, this->assets.coloredGlassTexture.view, VK_IMAGE_LAYOUT_SHADER_READ_ONLY_OPTIMAL };
...
void gltf_viewer::BuildCommandBuffers() {
...
void gltf_viewer::RebuildCommandBuffers(int frame) {
...
void gltf_viewer::PreTransformModel() {
	...
	//glm::mat4 translationMatrix = glm::translate(glm::mat4(1.0f), glm::vec3(-10.0f, 1.0f, 0.0f));
...
void gltf_viewer::CreateGeometryNodesBuffer() {
...
void gltf_viewer::UpdateDescriptorSet() {
