...
// -- map memory range
//@return VkResult
VkResult Buffer::map(VkDevice logicalDevice, VkDeviceSize size,
                     VkDeviceSize offset) {
  return vkMapMemory(logicalDevice, bufferData.memory, offset, size, 0,
                     &bufferData.mapped);
}
...
VkResult Buffer::bind(VkDevice logicalDevice, VkDeviceSize offset) {
  return vkBindBufferMemory(logicalDevice, bufferData.buffer, bufferData.memory,
                            offset);
}
...
VkResult Buffer::flush(VkDevice logicalDevice, VkDeviceSize size,
                       VkDeviceSize offset) {
...
VkResult Buffer::invalidate(VkDevice logicalDevice, VkDeviceSize size,
                            VkDeviceSize offset) {
  VkMappedMemoryRange mappedRange = {};
  mappedRange.sType = VK_STRUCTURE_TYPE_MAPPED_MEMORY_RANGE;
  mappedRange.memory = bufferData.memory;
...
//  return i }
//
uint32_t Buffer::findMemoryTypeIndex(VkPhysicalDevice physicalDevice,
...
VkBuffer Buffer::CreateBuffer(VkPhysicalDevice physicalDevice,
                              VkDevice logicalDevice,
                              VkBufferUsageFlags usageFlags,
                              VkDeviceSize bufferSize, std::string bufferName) {
  ...
  // create buffer
  if (vkCreateBuffer(logicalDevice, &bufferCreateInfo, nullptr,
...
VkDeviceMemory Buffer::AllocateBufferMemory(VkPhysicalDevice physicalDevice,
                                            VkDevice logicalDevice,
  ...
  memAlloc.memoryTypeIndex = findMemoryTypeIndex(
      physicalDevice, memReqs.memoryTypeBits, bufferData.memoryPropertyFlags);
  ...
  // allocate buffer memory
  if (vkAllocateMemory(logicalDevice, &memAlloc, nullptr, &bufferData.memory) !=
      VK_SUCCESS) {
...
// -- bind buffer memory
//@return VkResult
VkResult Buffer::BindBufferMemory(VkDevice logicalDevice,
                                  void *data = nullptr) {
  ...
  if (data != nullptr) {
    ...
    if ((bufferData.memoryPropertyFlags &
