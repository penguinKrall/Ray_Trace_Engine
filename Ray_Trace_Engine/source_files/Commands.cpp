...
VkCommandPool Commands::createCommandPool(VkDevice logicalDevice,
  ...
  if (vkCreateCommandPool(logicalDevice, &poolInfo, nullptr,
                          &this->commandPools.graphics) != VK_SUCCESS) {
...
VkCommandPool Commands::CreateComputeCommandPool(VkDevice logicalDevice,
                                                 uint32_t queueFamilyIndex) {
  // create info
  VkCommandPoolCreateInfo poolInfo = {};
  poolInfo.sType = VK_STRUCTURE_TYPE_COMMAND_POOL_CREATE_INFO;
  ...
  // create
  if (vkCreateCommandPool(logicalDevice, &poolInfo, nullptr,
                          &this->commandPools.compute) != VK_SUCCESS) {
...
VkQueue Commands::VKCreateQueue(VkDevice logicalDevice,
...
Commands::createGraphicsCommandBuffers(VkDevice logicalDevice,
  ...
  cbAllocInfo.commandBufferCount =
      static_cast<uint32_t>(this->commandBuffers.graphics.size());

  if (vkAllocateCommandBuffers(logicalDevice, &cbAllocInfo,
                               this->commandBuffers.graphics.data()) !=
      VK_SUCCESS) {
    throw std::invalid_argument(
        "Failed to allocate core Graphics Command Buffers!");
  }
...
Commands::CreatePresentSemaphore(VkDevice logicalDevice) {
  ...
  for (int i = 0; i < frame_draws; i++) {
    if (vkCreateSemaphore(logicalDevice, &semaphoreCreateInfo, nullptr,
                          &sync.presentFinishedSemaphore[i]) != VK_SUCCESS) {
...
std::vector<VkSemaphore>
Commands::CreateRenderSemaphore(VkDevice logicalDevice) {
  ...
  for (int i = 0; i < frame_draws; i++) {
    if (vkCreateSemaphore(logicalDevice, &semaphoreCreateInfo, nullptr,
                          &sync.renderFinishedSemaphore[i]) != VK_SUCCESS) {
...
std::vector<VkSemaphore>
Commands::CreateComputeSemaphore(VkDevice logicalDevice) {
  VkSemaphoreCreateInfo semaphoreCreateInfo = {};
  semaphoreCreateInfo.sType = VK_STRUCTURE_TYPE_SEMAPHORE_CREATE_INFO;
  semaphoreCreateInfo.pNext = nullptr;
  ...
  for (int i = 0; i < frame_draws; i++) {
    if (vkCreateSemaphore(logicalDevice, &semaphoreCreateInfo, nullptr,
                          &sync.computeFinishedSemaphore[i]) != VK_SUCCESS) {
      throw std::invalid_argument(
...
std::vector<VkFence> Commands::CreateFences(VkDevice logicalDevice) {
  ...
  for (auto &fence : sync.drawFences) {
    if (vkCreateFence(logicalDevice, &fenceCreateInfo, nullptr, &fence) !=
        VK_SUCCESS) {
...
void Commands::DestroyCommands(VkDevice logicalDevice) {
  ...
  for (int i = 0; i < frame_draws; i++) {
    ...
    vkDestroySemaphore(logicalDevice, sync.presentFinishedSemaphore[i],
                       nullptr);
    vkDestroySemaphore(logicalDevice, sync.computeFinishedSemaphore[i],
                       nullptr);
  }
