...
VKAPI_ATTR VkBool32 VKAPI_CALL CoreDebug::debugCallback(
	...
	if (pCallbackData->objectCount > 0 &&
		pCallbackData->pObjects != nullptr &&
		pCoreDebug != nullptr &&
		(isError || isWarning)) {
	...
	return VK_FALSE; // Returning VK_FALSE allows the Vulkan call to continue after the callback
}
...
void CoreDebug::initDebugCreateInfo() {
	debugMessenger.debugCreateInfo.sType = VK_STRUCTURE_TYPE_DEBUG_UTILS_MESSENGER_CREATE_INFO_EXT;
	debugMessenger.debugCreateInfo.flags = 0;
	debugMessenger.debugCreateInfo.messageSeverity = VK_DEBUG_UTILS_MESSAGE_SEVERITY_VERBOSE_BIT_EXT |
		VK_DEBUG_UTILS_MESSAGE_SEVERITY_WARNING_BIT_EXT | VK_DEBUG_UTILS_MESSAGE_SEVERITY_ERROR_BIT_EXT;
	debugMessenger.debugCreateInfo.messageType = VK_DEBUG_UTILS_MESSAGE_TYPE_GENERAL_BIT_EXT |
...
VkDebugUtilsMessengerEXT CoreDebug::createDebugMessenger(VkInstance instance) {
	...
	if (debugpFunctions.vkCreateDebugUtilsMessengerEXT(
		instance, &debugMessenger.debugCreateInfo, nullptr, &debugMessenger.debugMessenger) != VK_SUCCESS) {
		throw std::invalid_argument("Failed to create Debug Utilities Extension Messenger");
	}
...
bool CoreDebug::enumerateProperties() {
	...
	vkEnumerateInstanceLayerProperties(&debugSettings.instanceLayerCount, nullptr);

	//resize
	debugSettings.instanceLayerProperties.resize(debugSettings.instanceLayerCount);

	//get properties
	vkEnumerateInstanceLayerProperties(&debugSettings.instanceLayerCount, debugSettings.instanceLayerProperties.data());
	...
	for (VkLayerProperties layer : debugSettings.instanceLayerProperties) {
		if (nameIDX <= debugSettings.validationLayerName.size() &&
			strcmp(layer.layerName, debugSettings.validationLayerName[nameIDX]) == 0) {
...
void CoreDebug::destroyDebug(VkInstance instance) {
	debugpFunctions.vkDestroyDebugUtilsMessengerEXT(instance, debugMessenger.debugMessenger, nullptr);
}