...
public:
  ...
  //@param Example: add([this]() { return
  // functionThatReturnsObject(functionParameters); }, "objectName");
  template <typename Func>
  inline void add(Func createFunc, const std::string &name) {
    using HandleType = decltype(createFunc());
    HandleType handle =
        createFunc(); // Call the creation function to get the handle

    // Assuming HandleType has a member 'handle' or similar that holds the
    // actual handle
  ...
  //@brief Example: ([this]() { return memberFunction(function params); },
  // function return type, "objectName");
  template <typename Func, typename HandleType>
  inline void addArray(Func createFunc, HandleType type,
