...
float imguiGizmo::cylRadius =
    0.02f; // sizeCylLength = defined in base to control size
...
// Sphere Colors
///////////////////////////////////////
ImU32 imguiGizmo::sphereColors[2] = {0xff401010,
                                     0xffc0a0a0}; // Tessellation colors
...
///////////////////////////////////////
float imguiGizmo::gizmoFeelingRot =
    1.f; // >1 more mouse sensibility, <1 less mouse sensibility
...
vgModifiers imguiGizmo::panMod = vg::evControlModifier,
            imguiGizmo::dollyMod = vg::evShiftModifier;
#endif
...
//          section "settings of class declaration", these these values are
//          valid for ALL controls in your application, because the lists of
//          triangles/quads, which compose the solids, are builded one time with
//          the first instance ... and NOT every redraw
//
...
bool gizmo3D(const char *label, vec3 &dir, float size, const int mode) {
  imguiGizmo g;
  g.modeSettings(mode & (imguiGizmo::modeDirection | imguiGizmo::modeDirPlane)
                     ? mode
...
////////////////////////////////////////////////////////////////////////////
bool gizmo3D(const char *label, quat &axes, quat &spot, float size,
             const int mode) {
  imguiGizmo g;
  g.setDualMode(mode);
...
////////////////////////////////////////////////////////////////////////////
bool gizmo3D(const char *label, quat &axes, vec3 &spotDir, float size,
             const int mode) {
  imguiGizmo g;
  g.setDualMode(mode);
...
////////////////////////////////////////////////////////////////////////////
bool gizmo3D(const char *label, quat &axes, vec4 &axesAngle, float size,
             const int mode) {
  imguiGizmo g;
  g.setDualMode(mode);
...
//          - quat (quaternion) rotation
////////////////////////////////////////////////////////////////////////////
bool gizmo3D(const char *label, vec3 &vPanDolly, quat &q, float size,
             const int mode) {
  imguiGizmo g;
...
//          - vec4 - X Y Z vector/axes components - W angle of rotation
////////////////////////////////////////////////////////////////////////////
bool gizmo3D(const char *label, vec3 &vPanDolly, vec4 &axis_angle, float size,
             const int mode) {
  imguiGizmo g;
...
//          - vec3 - X Y Z vector/axes components
////////////////////////////////////////////////////////////////////////////
bool gizmo3D(const char *label, vec3 &vPanDolly, vec3 &dir, float size,
             const int mode) {
  imguiGizmo g;
  g.modeSettings(mode & ((imguiGizmo::modeDirection | imguiGizmo::modeDirPlane)
                             ? mode
...
//                are abilitated on both ... also together!
////////////////////////////////////////////////////////////////////////////
bool gizmo3D(const char *label, vec3 &vPanDolly, quat &axes, quat &spot,
             float size, const int mode) {
  imguiGizmo g;
  g.setDualMode(mode);
  g.posPanDolly = vPanDolly;
...
//          - spot (vec3)       for full control - RightClick
////////////////////////////////////////////////////////////////////////////
bool gizmo3D(const char *label, vec3 &vPanDolly, quat &axes, vec3 &spotDir,
             float size, const int mode) {
  imguiGizmo g;
  g.setDualMode(mode);
  g.posPanDolly = vPanDolly;
...
//          - spot (vec4)       for full control - RightClick
////////////////////////////////////////////////////////////////////////////
bool gizmo3D(const char *label, vec3 &vPanDolly, quat &axes, vec4 &axesAngle,
             float size, const int mode) {
  imguiGizmo g;
  g.setDualMode(mode);
  g.posPanDolly = vPanDolly;
...
inline ImU32 addLightEffect(const vec4 &color, float light, float atten) {
  ...
  vec3 c(((vec3(color) + l * .5f) * l) * .75f + a * vec3(color) * .45f +
         a * .25f);

  const float alpha =
      color.a * ImGui::GetStyle().Alpha; // ImGui::GetCo(ImGuiCol_FrameBg).w;
...
inline ImU32 addLightEffect(ImU32 color, float light, float atten) {
  vec4 c(float(color & 0xff) / 255.f, float((color >> 8) & 0xff) / 255.f,
         float((color >> 16) & 0xff) / 255.f, 1.0f);
...
inline vec3 &adjustPlane(vec3 &coord) {
  ...
  coord.x =
      (coord.x) * .5f + .5f +
      (coord.x > 0 ? -imguiGizmo::planeThickness : imguiGizmo::planeThickness) *
          imguiGizmo::solidResizeFactor;
...
inline vec3 &adjustSpotCyl(vec3 &coord) {
  ...
  coord.x = (coord.x * .075f - 2.0f +
             (halfCylMinusCone -
              halfCylMinusCone * .075f)); // cyl begin where cone end
  return coord;
}
...
inline vec3 fastRotate(int axis, vec3 &v) {
  return ((axis == imguiGizmo::axisIsY) ? vec3(-v.y, v.x, v.z)
                                        : // rotation Z 90'
              ((axis == imguiGizmo::axisIsZ) ? vec3(-v.z, v.y, v.x)
                                             : // rotation Y 90'
                   v));
}
////////////////////////////////////////////////////////////////////////////
...
bool imguiGizmo::drawFunc(const char *label, float size) {
  ...
  const float arrowStartingPoint =
      (axesOriginType & imguiGizmo::sphereAtOrigin)
          ...
          : ((axesOriginType & imguiGizmo::cubeAtOrigin)
                 ? cubeSize * solidResizeFactor
                 : cylRadius * .5f);
  // if modeDual... leave space for draw light arrow
  vec3 resizeAxes(((drawMode & modeDual) && (axesResizeFactor.x > .75f))
                      ? vec3(.75f, axesResizeFactor.y, axesResizeFactor.z)
                      : axesResizeFactor);
  ...
  auto getTrackball = [&](quat &q) {
    ...
    if (drawMode & modePanDolly || io.MouseWheel != 0) {
      float screenFactor =
          1.f / (io.DisplaySize.x < io.DisplaySize.y ? io.DisplaySize.x
                                                     : io.DisplaySize.y);
      ...
      track.motionImmediateMode(mouse.x, mouse.y, io.MouseDelta.x,
                                io.MouseDelta.y, vgMods);
      // get new rotation only if !Pan && ! Dolly
      if ((!track.isDollyActive() && !track.isPanActive() &&
           io.MouseWheel == 0))
        q = track.getRotation();
      else
        posPanDolly = track.getPosition();
    } else {
      track.imGuIZMO_BASE_CLASS::motionImmediateMode(
          mouse.x, mouse.y, io.MouseDelta.x, io.MouseDelta.y, vgMods);
      q = track.getRotation();
    }
#else
    track.motionImmediateMode(mouse.x, mouse.y, io.MouseDelta.x,
  ...
  if (ImGui::IsItemActive()) {
    ...
    if (ImGui::IsMouseDragging(0))
      getTrackball(qtV);
    if ((drawMode & modeDual) && ImGui::IsMouseDragging(1))
      getTrackball(qtV2); // if dual mode... move together
    // if((drawMode&modeDual) && ImGui::IsMouseDragging(2)) { getTrackball(qtV);
    // getTrackball(qtV2); } // middle if dual mode... move together

    ImColor col(style.Colors[ImGuiCol_FrameBgActive]);
    col.Value.w *= ImGui::GetStyle().Alpha;
    draw_list->AddRectFilled(controlPos, controlPos + innerSize, col,
                             style.FrameRounding);
    ...
    if (highlighted && (drawMode & modeDual) && ImGui::IsMouseDragging(1))
      getTrackball(qtV2);
    else if (highlighted && (drawMode & modeDual) &&
             ImGui::IsMouseDragging(2)) {
      getTrackball(qtV);
      getTrackball(qtV2);
    }
#ifndef IMGUIZMO_USE_ONLY_ROT
    else if (highlighted && io.MouseWheel != 0)
      getTrackball(qtV);
#endif

    ImColor col(highlighted ? style.Colors[ImGuiCol_FrameBgHovered]
  ...
  ImVec2 uv[4];
  ImU32 col[4]; // buffers to storetransformed vtx & col for PrimVtx &
                // PrimQuadUV
  ...
  auto normalizeToControlSize = [&](float x, float y) {
    return controlPos + ImVec2(x, -y) * halfSquareSize +
           ImVec2(halfSquareSize,
                  halfSquareSize); // drawing from 0,0 .. no borders
  };
  ...
  auto addQuad = [&](ImU32 colLight) { // test cull dir
    ...
    draw_list->PrimQuadUV(uv[0], uv[1], uv[2], uv[3], wpUV, wpUV, wpUV, wpUV,
                          colLight);
  };

  //////////////////////////////////////////////////////////////////
  auto drawSphere = [&]() {
    draw_list->PrimReserve(sphereVtx.size(),
                           sphereVtx.size()); // num vert/indices
    ...
    for (auto itVtx = sphereVtx.begin(); itVtx != sphereVtx.end();) {
      for (int h = 0; h < 3; h++, itTess++) {
        ...
        col[h] = addLightEffect(
            sphereColors[*itTess],
            (-drawSize * .5f + (coord.z * coord.z) / (drawSize * drawSize)));
        // col[h] = colorLightedY(sphereCol[i++], (-sizeSphereRadius.5f +
        // (coord.z*coord.z) / (sizeSphereRadius*sizeSphereRadius)), coord.z);
      }
      addTriangle();
    }
  };
  ...
  //////////////////////////////////////////////////////////////////
  auto drawCube = [&]() {
    draw_list->PrimReserve(cubeNorm.size() * 6,
                           cubeNorm.size() * 4); // num vert/indices
    for (vec3 *itNorm = cubeNorm.begin(), *itVtx = cubeVtx.begin();
         itNorm != cubeNorm.end();) {
      vec3 coord;
      vec3 norm = _q * *itNorm;
      for (int i = 0; i < 4;) {
  ...
  //////////////////////////////////////////////////////////////////
  auto drawPlane = [&]() {
    draw_list->PrimReserve(planeNorm.size() * 6,
                           planeNorm.size() * 4); // num vert/indices
    for (auto itNorm = planeNorm.begin(), itVtx = planeVtx.begin();
         itNorm != planeNorm.end();) {
      vec3 coord;
      vec3 norm = _q * *itNorm;
      for (int i = 0; i < 4;) {
        coord = _q * (*itVtx++ * solidResizeFactor);
      ...
      itNorm++;
      addQuad(addLightEffect(
          vec4(planeColor.x, planeColor.y, planeColor.z, planeColor.w), norm.z,
          coord.z));
    }
  };
  ...
  auto drawAxes = [&](int side) {
    for (int n = 0; n < 4;
         n++) { // Arrow: 2 Cone -> (Surface + cap) + 2 Cyl -> (Surface + cap)
      for (int arrowAxis = 0; arrowAxis < 3; arrowAxis++) { // draw 3 axes
        vec3 arrowCoord(0.0f, 0.0f, 0.0f);
        arrowCoord[arrowAxis] = 1.0f; // rotate on 3 axis (arrow -> X, Y, Z ) in
                                      // base to current arrowAxis
        const float arrowCoordZ = vec3(_q * arrowCoord).z; //.Rotate

        const int i =
            (arrowCoordZ > 0) ? 3 - n : n; // painter algorithm: before farthest

        bool skipCone = true;

        if ((side == backSide && arrowCoordZ > 0) ||
            (side == frontSide && arrowCoordZ <= 0)) {
          if (!showFullAxes && (i == CYL_CAP))
            continue; // skip if cylCap is hidden
          if (i <= CONE_CAP)
        ...
        draw_list->PrimReserve(ptrVtx->size(),
                               ptrVtx->size()); // // reserve vtx

        for (auto itVtx = ptrVtx->begin(), itNorm = (arrowNorm + i)->begin();
             itVtx != ptrVtx->end();) { // for all Vtx
          ...
          for (int h = 0; h < 3; h++) {
            ...
            if ((skipCone && coord.x <= 0) ||
                (!showFullAxes && (coord.x < arrowStartingPoint)))
              coord.x = arrowStartingPoint;
            // transform
            ...
            vec3 norm(_q * fastRotate(arrowAxis, *itNorm++));
#endif
            // col[h] = addLightEffect(ImU32(0xFF) << arrowAxis*8,
            // float(0xa0)*norm.z+.5f);
            col[h] = addLightEffect(vec4(float(arrowAxis == axisIsX),
                                         float(arrowAxis == axisIsY),
                                         float(arrowAxis == axisIsZ), 1.0),
                                    norm.z, coord.z);
          }
          addTriangle();
        }
  ...
  auto drawComponent = [&](const int idx, const quat &q, ptrFunc func) {
    ...
    for (auto itVtx = ptrVtx->begin(), itNorm = (arrowNorm + idx)->begin();
         itVtx != ptrVtx->end();) {
#if !defined(imguiGizmo_INTERPOLATE_NORMALS)
      vec3 norm = (_q * *itNorm++);
#endif
      for (int h = 0; h < 3; h++) {
        ...
        vec3 norm = (q * *itNorm++);
#endif
        coord = q * (func(coord) * resizeAxes); // remodelling Directional Arrow
                                                // (func) and transforms;

        uv[h] = normalizeToControlSize(coord.x, coord.y);
        // col[h] = addLightEffect(color, float(0xa0)*norm.z+.5f);
        col[h] = addLightEffect(
            vec4(directionColor.x, directionColor.y, directionColor.z, 1.0),
            norm.z, coord.z > 0 ? coord.z : coord.z * .5f);
      }
      addTriangle();
    }
  };
...
#define CENTER_HELPER_Y -.85f
  ...
  auto drawRotationHelper = [&]() {
    const ImVec2 center(
        normalizeToControlSize(CENTER_HELPER_X, CENTER_HELPER_Y));
    ...
    if (squareSize < 100) { // if too small filled circle
      ...
      const float a_max =
          (IM_PI * 1.5f) * ((float)nSegments) / (float)nSegments;
      draw_list->PathClear();
  ...
  auto drawPanHelper = [&]() {
    ...
    draw_list->AddTriangleFilled(
        ...
        ImVec2(center.x + halfLen, center.y + lenLine - hhLen), color);
    draw_list->AddTriangleFilled(
        ImVec2(center.x, center.y - lenLine - halfLen),
        ImVec2(center.x - halfLen, center.y - lenLine + hhLen),
        ImVec2(center.x + halfLen, center.y - lenLine + hhLen), color);
    draw_list->AddTriangleFilled(
        ...
        ImVec2(center.x + lenLine - hhLen, center.y + halfLen), color);
    draw_list->AddTriangleFilled(
        ImVec2(center.x - lenLine - halfLen, center.y),
        ImVec2(center.x - lenLine + hhLen, center.y - halfLen),
        ImVec2(center.x - lenLine + hhLen, center.y + halfLen), color);
  ...
  ///////////////////////////////////////
  // if((drawMode & modePanDolly) && (ImGui::IsItemHovered() ||
  ...
  if (vgModsActive && (ImGui::IsItemHovered() &&
...
//  Polygon
////////////////////////////////////////////////////////////////////////////
void imguiGizmo::buildPolygon(const vec3 &size, ImVector<vec3> &vtx,
                              ImVector<vec3> &norm) {
...
void imguiGizmo::buildSphere(const float radius, const int tessFactor) {
  const int div =
      tessFactor;           // tessellation colors: meridians/div x paralles/div
...
#define T(t) sphereTess.push_back(t)
  ...
  // Cover each stack with a quad divided in 2 triangles, except the top and
  // bottom stacks
  ...
  for (int j = 0; j < meridians; j++, angle += incAngle) {
    ...
    const int tType =
        ((parallels - 1) >> div) & 1 ? ((j >> div) & 1) : !((j >> div) & 1);
...
//  Cone / Pyramid
////////////////////////////////////////////////////////////////////////////
void imguiGizmo::buildCone(const float x0, const float x1, const float radius,
                           const int slices) {
  ...
  float yt1 = sinn,
        y1 = radius;           // ==> yt1 = cos(0) * sinn, y1 = cos(0) * radius
...
//  Cylinder
////////////////////////////////////////////////////////////////////////////
void imguiGizmo::buildCylinder(const float x0, const float x1,
