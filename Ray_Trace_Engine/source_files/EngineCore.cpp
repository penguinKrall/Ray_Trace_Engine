...
VkInstance EngineCore::createInstance(bool enableValidation) {
	...
	if (extCount > 0) {
		...
		if (vkEnumerateInstanceExtensionProperties(nullptr, &extCount, extensions.data()) == VK_SUCCESS) {
			...
			for (const auto& extension : extensions) {
				...
				std::cout << " [" << extIDX << "] " << supportedInstanceExtensions.back() << std::endl;
				extIDX++;
			}
		}
	}
	...
	enabledExtensions.reserve(supportedInstanceExtensions.size()); // Reserve space for efficiency
	...
	if (baseSettings.validation) {
		if (enumerateProperties()) {
			...
			std::cout << std::endl << "validation layer: " << *getValidationLayerNames()
				<< "  ...initializing validation " << std::endl << std::endl;
		}
	...
	if (vkCreateInstance(&instanceCreateInfo, nullptr, &instance) != VK_SUCCESS) {
		throw std::invalid_argument("Failed to create instance!");
	}
...
void EngineCore::InitCore() {
	...
	if (getPhysicalDevice(instance) == VK_SUCCESS) {
		std::cout << std::endl << "main: found and selected supported GPU" << "\n" << std::endl;
	}
	...
	//initialize camera
	camera = new Camera(glm::vec3(0.0f, 0.0f, 20.0f), glm::vec3(0.0f, 1.0f, 0.0f), windowGLFW);
	...
	//create command pool
	add([this]() { return createCommandPool(devices.logical, queue.queueFamilyIndices.graphics); }, "commandPool");
	...
	addArray([this]() {
		return createGraphicsCommandBuffers(devices.logical, devices.physical); }, VkCommandBuffer{}, "graphicsCommandBuffer");
...
void EngineCore::CreateQueues() {
	...
	add([this]() { return VKCreateQueue(
		devices.logical, queue.queueFamilyIndices.compute, 0, &queue.compute); }, "computeQueue");

	add([this]() { return VKCreateQueue(
		devices.logical, queue.queueFamilyIndices.present, 0, &queue.present); }, "presentQueue");

	add([this]() { return VKCreateQueue(
		devices.logical, queue.queueFamilyIndices.transfer, 0, &queue.transfer); }, "transferQueue");
...
void EngineCore::CreateCoreSwapchain() {
	...
	// -- swapchain images
	addArray([this]() { return createSwapchainImage(devices.logical); }, VkImage{}, "swapchainImage");
...
void EngineCore::CreateSyncObjects() {
	...
	//
	//add([this]() {
	//	return CreateRenderSemaphore(devices.logical); }, "RenderFinishedSemaphore");
	//
	//add([this]() {
	//	return CreateComputeSemaphore(devices.logical); }, "ComputeSemaphore");

	//semaphores
	addArray([this]() { return CreatePresentSemaphore(devices.logical); }, VkSemaphore{}, "presentSemaphores");

	addArray([this]() { return CreateRenderSemaphore(devices.logical); }, VkSemaphore{}, "renderFinishedSemaphores");

	addArray([this]() { return CreateComputeSemaphore(devices.logical); }, VkSemaphore{}, "computeSemaphores");

	//fences
	addArray([this]() { return CreateFences(devices.logical); }, VkFence{}, "syncFences");

	//fences
	addArray([this]() { return CreateComputeFences(devices.logical); }, VkFence{}, "syncComputeFences");
...
void EngineCore::RecreateSyncObjects() {
	...
	for (int i = 0; i < frame_draws; i++) {
		vkDestroySemaphore(this->devices.logical, sync.renderFinishedSemaphore[i], nullptr);
		vkDestroySemaphore(this->devices.logical, sync.presentFinishedSemaphore[i], nullptr);
		vkDestroySemaphore(this->devices.logical, sync.computeFinishedSemaphore[i], nullptr);
	}
...
void EngineCore::CreateLoadingScreenImage(const std::string& fileName) {
	...
	if (pEngineCore->CreateBuffer(
		...
		imageSize,
		image) != VK_SUCCESS) {
		throw std::invalid_argument("failed to create load_screen_image_staging buffer");
	}
	...
	imageCreateInfo.format = this->swapchainData.assignedSwapchainImageFormat.format;
...
void EngineCore::LoadingScreen() {
	...
	vkWaitForFences(devices.logical, 1, &sync.drawFences[0], VK_TRUE, std::numeric_limits<uint64_t>::max());
	...
	submitInfo.waitSemaphoreCount = graphicsSemaphores.size();
	...
	if (vkQueueSubmit(queue.graphics, 1, &submitInfo, sync.drawFences[0]) != VK_SUCCESS) {
