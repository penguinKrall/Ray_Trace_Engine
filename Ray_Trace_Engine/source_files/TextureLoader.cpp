...
ktxResult TextureLoader::loadKTXFile(std::string filename,
                                     ktxTexture **target) {
...
void TextureLoader::loadFromFile(std::string filename, VkFormat format,
  ...
  if (useStaging) {
    ...
    pEngineCore->add(
        ...
        "ktxLoadImageStagingBuffer" + extractedFilename);
    ...
    bufferMemoryAllocateInfo.memoryTypeIndex =
        pEngineCore->getMemoryType(bufferMemoryRequirements.memoryTypeBits,
    ...
    pEngineCore->add(
        ...
        "ktxLoadImageStagingBufferMemory" + extractedFilename);

    // bind staging buffer and memory
    if (vkBindBufferMemory(pEngineCore->devices.logical, stagingBuffer,
    ...
    // map memory
    if (vkMapMemory(pEngineCore->devices.logical, stagingMemory, 0,
                    bufferMemoryRequirements.size, 0,
    ...
    pEngineCore->add(
        ...
        "ktxTextureImage" + extractedFilename);
    ...
    pEngineCore->add(
        ...
        "ktxTextureImageMemory" + extractedFilename);

    if (vkBindImageMemory(pEngineCore->devices.logical, image, imageMemory,
                          0) != VK_SUCCESS) {
      throw std::invalid_argument("failed to ");
    }
    ...
    gtp::Utilities_EngCore::setImageLayout(copyCmd, image,
                                           VK_IMAGE_LAYOUT_TRANSFER_DST_OPTIMAL,
    ...
    pEngineCore->add(
        ...
        "ktxTextureImageMappable" + extractedFilename);
    ...
    vkGetImageMemoryRequirements(pEngineCore->devices.logical, mappableImage,
                                 &imageMemoryRequirements);
    ...
    pEngineCore->add(
        ...
        "ktxTextureImageMemoryMappable" + extractedFilename);

    // Bind allocated image for use
    if (vkBindImageMemory(pEngineCore->devices.logical, mappableImage,
                          mappableMemory, 0) != VK_SUCCESS) {
      throw std::invalid_argument("failed to ");
    }
    ...
    vkGetImageSubresourceLayout(pEngineCore->devices.logical, mappableImage,
                                &subRes, &subResLayout);

    // map image memory
    if (vkMapMemory(pEngineCore->devices.logical, mappableMemory, 0,
                    imageMemoryRequirements.size, 0, &data) != VK_SUCCESS) {
      throw std::invalid_argument("failed to ");
    }
  ...
  pEngineCore->add(
      ...
      "ktxTextureImageSampler" + extractedFilename);
  ...
  pEngineCore->add(
      ...
      "ktxTextureImageView" + extractedFilename);
...
void TextureLoader::fromglTfImage(tinygltf::Image &gltfimage, std::string path,
  ...
  if (!isKtx) {
    ...
    if (vkCreateBuffer(pEngineCore->devices.logical, &bufferCreateInfo, nullptr,
    ...
    if (vkMapMemory(pEngineCore->devices.logical, stagingMemory, 0,
                    memReqs.size, 0, (void **)&data) != VK_SUCCESS) {
    ...
    if (vkCreateBuffer(pEngineCore->devices.logical, &bufferCreateInfo, nullptr,
      ...
      throw std::invalid_argument(
          "failed to create gltx texture staging buffer");
    }
    ...
    if (vkAllocateMemory(pEngineCore->devices.logical, &memAllocInfo, nullptr,
                         &stagingMemory) != VK_SUCCESS) {
      throw std::invalid_argument(
          "failed to allocate gltx texture staging buffer memory");
    }
    ...
    if (vkBindBufferMemory(pEngineCore->devices.logical, stagingBuffer,
                           stagingMemory, 0) != VK_SUCCESS) {
      throw std::invalid_argument(
          "failed to bind gltx texture staging buffer and memory");
    }
    ...
    if (vkAllocateMemory(pEngineCore->devices.logical, &memAllocInfo, nullptr,
                         &imageMemory) != VK_SUCCESS) {
      throw std::invalid_argument(
          "failed to allocate gltx texture image memory");
    }

    if (vkBindImageMemory(pEngineCore->devices.logical, image, imageMemory,
                          0) != VK_SUCCESS) {
      throw std::invalid_argument(
          "failed to bind gltx texture image and memory");
    }
    ...
    pEngineCore->FlushCommandBuffer(
        copyCmd, copyQueue, this->pEngineCore->commandPools.graphics, true);
  ...
  if (vkCreateSampler(pEngineCore->devices.logical, &samplerInfo, nullptr,
                      &sampler) != VK_SUCCESS) {
  ...
  if (vkCreateImageView(pEngineCore->devices.logical, &viewInfo, nullptr,
                        &view) != VK_SUCCESS) {
...
void TextureLoader::LoadCubemap() {
  ...
  std::vector<stbi_uc *> imageData = {
      {this->LoadTextureFile(
          ...
          "/assets/textures/cubemaps/desert_grass_cubemap_2/px.png", &width,
          &height, &imageSize)},
      {this->LoadTextureFile(

          "/assets/textures/cubemaps/desert_grass_cubemap_2/nx.png", &width,
          &height, &imageSize)},
      {this->LoadTextureFile(

          "/assets/textures/cubemaps/desert_grass_cubemap_2/py.png", &width,
          &height, &imageSize)},
      {this->LoadTextureFile(

          "/assets/textures/cubemaps/desert_grass_cubemap_2/ny.png", &width,
          &height, &imageSize)},
      {this->LoadTextureFile(

          "/assets/textures/cubemaps/desert_grass_cubemap_2/pz.png", &width,
          &height, &imageSize)},
      {this->LoadTextureFile(

          "/assets/textures/cubemaps/desert_grass_cubemap_2/nz.png", &width,
          &height, &imageSize)}};
