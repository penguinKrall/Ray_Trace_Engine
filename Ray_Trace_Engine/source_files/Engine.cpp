		...
		for (int i = 0; i < this->renderers.mainRenderer.gltfCompute.size(); i++) {
			if (this->renderers.mainRenderer.gltfCompute[i] != nullptr) {
				...
				validate_vk_result(vkResetCommandBuffer(
					this->renderers.mainRenderer.gltfCompute[i]->commandBuffers[currentFrame],
				...
				this->renderers.mainRenderer.gltfCompute[i]->RecordComputeCommands(currentFrame);

				// end compute command buffer
				validate_vk_result(vkEndCommandBuffer(
					this->renderers.mainRenderer.gltfCompute[i]->commandBuffers[currentFrame]));
				computeCommands.push_back(this->renderers.mainRenderer.gltfCompute[i]->commandBuffers[currentFrame]);
			}
		}
