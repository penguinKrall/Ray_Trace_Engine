...
uint32_t CoreDevice::getMemoryType(uint32_t typeBits, VkMemoryPropertyFlags properties, VkBool32* memTypeFound) const {
	for (uint32_t i = 0; i < deviceProperties.memory.memoryTypeCount; i++) {
		if ((typeBits & 1) == 1) {
			if ((deviceProperties.memory.memoryTypes[i].propertyFlags & properties) == properties) {
				if (memTypeFound)
				{
					*memTypeFound = true;
				}
...
VkResult CoreDevice::getPhysicalDevice(VkInstance instance) {
	...
	if (vkEnumeratePhysicalDevices(instance, &deviceCount, nullptr) != VK_SUCCESS) {
		throw std::invalid_argument("Can't find GPUs that support Vulkan Instance!");
	}
	else {
	...
	if (vkEnumeratePhysicalDevices(instance, &deviceCount, deviceData.deviceList.data())) {
	...
	vkGetPhysicalDeviceProperties(devices.physical, &deviceProperties.physicalDevice);
	vkGetPhysicalDeviceFeatures(devices.physical, &deviceData.features);
	vkGetPhysicalDeviceMemoryProperties(devices.physical, &deviceProperties.memory);
	...
	std::cout << "Memory Types: " << deviceProperties.memory.memoryTypeCount << std::endl;
	std::cout << "Memory Heaps: " << deviceProperties.memory.memoryHeapCount << std::endl;

	for (int i = 0; i < static_cast<int>(deviceProperties.memory.memoryTypeCount); i++) {
		std::cout << "Memory Type [" << i << "]"  <<
			"\nMemory property flags: " << deviceProperties.memory.memoryTypes[i].propertyFlags << std::endl;
	}

	//get extension properties from device
	uint32_t extensionCount = 0;
	vkEnumerateDeviceExtensionProperties(devices.physical, nullptr, &extensionCount, nullptr);


	if (extensionCount != 0) {
		coreExtensions->deviceExtensions.properties.resize(extensionCount);
		vkEnumerateDeviceExtensionProperties(devices.physical, nullptr, &extensionCount, coreExtensions->deviceExtensions.properties.data());


		//output extension data
		std::cout << std::endl << "Physical Device Extension Properties:" << std::endl;
		for (const auto& extension : coreExtensions->deviceExtensions.properties) {
			coreExtensions->deviceExtensions.supported.emplace_back(extension.extensionName);
		...
		coreExtensions->deviceExtensions.enabled.reserve(coreExtensions->deviceExtensions.supported.size()); // Reserve space for efficiency
		...
		for (auto& str : coreExtensions->deviceExtensions.supported) {
			...
			if (!(strcmp(str.data(), "VK_NV_cuda_kernel_launch") == 0) &&
				!(strcmp(str.data(), VK_EXT_BUFFER_DEVICE_ADDRESS_EXTENSION_NAME) == 0) &&
		...
		std::cout << std::endl << "Physical Device Enabled Device Extension Count: " <<
			coreExtensions->deviceExtensions.supported.size() - skippedExtCount << std::endl;

		//for (int i = 0; i < coreExtensions->deviceExtensions.enabled.size(); i++) {
		//std::cout << " [" << i << "]" << coreExtensions->deviceExtensions.enabled[i] << std::endl;
		//}
	...
	deviceData.bufferDeviceAddressFeatures.sType = VK_STRUCTURE_TYPE_PHYSICAL_DEVICE_BUFFER_DEVICE_ADDRESS_FEATURES;
	deviceData.bufferDeviceAddressFeatures.bufferDeviceAddress = VK_TRUE;
	deviceData.bufferDeviceAddressFeatures.pNext = nullptr;

	deviceData.rayTracingPipelineFeatures.sType = VK_STRUCTURE_TYPE_PHYSICAL_DEVICE_RAY_TRACING_PIPELINE_FEATURES_KHR;
	deviceData.rayTracingPipelineFeatures.rayTracingPipeline = VK_TRUE;
	deviceData.rayTracingPipelineFeatures.pNext = &deviceData.bufferDeviceAddressFeatures;

	deviceData.accelerationStructureFeatures.sType = VK_STRUCTURE_TYPE_PHYSICAL_DEVICE_ACCELERATION_STRUCTURE_FEATURES_KHR;
	deviceData.accelerationStructureFeatures.accelerationStructure = VK_TRUE;
	deviceData.accelerationStructureFeatures.pNext = &deviceData.rayTracingPipelineFeatures;

	//// VK_KHR_ray_tracing_position_fetch has a new feature struct
	//deviceData.enabledRayTracingPositionFetchFeatures.sType = VK_STRUCTURE_TYPE_PHYSICAL_DEVICE_RAY_TRACING_POSITION_FETCH_FEATURES_KHR;
	//deviceData.enabledRayTracingPositionFetchFeatures.rayTracingPositionFetch = VK_TRUE;
	//deviceData.enabledRayTracingPositionFetchFeatures.pNext = &deviceData.accelerationStructureFeatures;

	deviceData.physicalDeviceDescriptorIndexingFeatures.sType = VK_STRUCTURE_TYPE_PHYSICAL_DEVICE_DESCRIPTOR_INDEXING_FEATURES_EXT;
	deviceData.physicalDeviceDescriptorIndexingFeatures.shaderSampledImageArrayNonUniformIndexing = VK_TRUE;
	deviceData.physicalDeviceDescriptorIndexingFeatures.runtimeDescriptorArray = VK_TRUE;
	deviceData.physicalDeviceDescriptorIndexingFeatures.descriptorBindingVariableDescriptorCount = VK_TRUE;
	deviceData.physicalDeviceDescriptorIndexingFeatures.pNext = &deviceData.accelerationStructureFeatures;


	deviceData.features2.sType = VK_STRUCTURE_TYPE_PHYSICAL_DEVICE_FEATURES_2;
	deviceData.features2.pNext = &deviceData.physicalDeviceDescriptorIndexingFeatures;
	...
	std::cout << "  Buffer Device Address: " <<
		(deviceData.bufferDeviceAddressFeatures.bufferDeviceAddress ? "Enabled" : "Disabled") << std::endl;

	std::cout << "  Ray Tracing Pipeline: " <<
		(deviceData.rayTracingPipelineFeatures.rayTracingPipeline ? "Enabled" : "Disabled") << std::endl;

	std::cout << "  Acceleration Structure: " <<
		(deviceData.accelerationStructureFeatures.accelerationStructure ? "Enabled" : "Disabled") << std::endl;
	...
	if (!rayTracingSupported || !accelerationStructureSupported || !bufferDeviceAddressSupported) {
	...
	std::cout << "  Buffer Device Address: " <<
		(deviceData.bufferDeviceAddressFeatures.bufferDeviceAddress ? "Supported" : "Not Supported") << std::endl;
	std::cout << "  Raytracing Pipeline: " <<
		(deviceData.rayTracingPipelineFeatures.rayTracingPipeline ? "Supported" : "Not Supported") << std::endl;
	std::cout << "  Acceleration Structure: " <<
		(deviceData.accelerationStructureFeatures.accelerationStructure ? "Supported" : "Not Supported") << std::endl;

	//properties
	//rt pipeline
	deviceProperties.rayTracingPipelineKHR.sType = VK_STRUCTURE_TYPE_PHYSICAL_DEVICE_RAY_TRACING_PIPELINE_PROPERTIES_KHR;

	//acceleration structure
	deviceProperties.accelerationStructureKHR.sType = VK_STRUCTURE_TYPE_PHYSICAL_DEVICE_ACCELERATION_STRUCTURE_PROPERTIES_KHR;

	//chain pNext from deviceProperties.rayTracingPipelineKHR
	deviceProperties.accelerationStructureKHR.pNext = &deviceProperties.rayTracingPipelineKHR;

	//get deviceProperties.physicalDevice2
	deviceProperties.physicalDevice2.sType = VK_STRUCTURE_TYPE_PHYSICAL_DEVICE_PROPERTIES_2;
	deviceProperties.physicalDevice2.pNext = &deviceProperties.accelerationStructureKHR;
	vkGetPhysicalDeviceProperties2(devices.physical, &deviceProperties.physicalDevice2);

	// Output ray tracing pipeline properties
	std::cout << std::endl << "Ray Tracing Pipeline Properties:" << std::endl;
	std::cout << "  Max Recursion Depth: " << deviceProperties.rayTracingPipelineKHR.maxRayRecursionDepth << std::endl;
	std::cout << "  Max Ray Hit Attribute Size: " << deviceProperties.rayTracingPipelineKHR.maxRayHitAttributeSize << std::endl;
	// Add more properties as needed

	// Output acceleration structure properties
	std::cout << std::endl << "Acceleration Structure Properties:" << std::endl;
	std::cout << "  Max Instance Count: " << deviceProperties.accelerationStructureKHR.maxInstanceCount << std::endl;
	std::cout << "  Max Geometry Count: " << deviceProperties.accelerationStructureKHR.maxGeometryCount << std::endl;
	std::cout << "  Max Descriptor Set Acceleration Structures: " <<
		deviceProperties.accelerationStructureKHR.maxDescriptorSetAccelerationStructures << std::endl;
	// Add more properties as needed
...
//	vkGetPhysicalDeviceQueueFamilyProperties(devices.physical, &queueFamilyCount, queueFamilyList.data());
//
//	//count
//	int idx = 0;
//
...
//		if (vkGetPhysicalDeviceSurfaceSupportKHR(devices.physical, idx, surface, &presentationSupport) != VK_SUCCESS) {
...
VkDevice CoreDevice::createLogicalDevice(VkSurfaceKHR surface) {
	...
	std::cout << " queueFamilyIndices.compute: " << queue.queueFamilyIndices.compute << std::endl;
	std::cout << " queueFamilyIndices.present: " << queue.queueFamilyIndices.present << std::endl;
	std::cout << " queueFamilyIndices.transfer: " << queue.queueFamilyIndices.transfer << std::endl;
	...
	deviceCreateInfo.enabledExtensionCount = static_cast<uint32_t>(coreExtensions->deviceExtensions.enabled.size());
	deviceCreateInfo.ppEnabledExtensionNames = coreExtensions->deviceExtensions.enabled.data();
	deviceCreateInfo.pNext = &deviceData.features2;

	// Output enabled extensions
	std::cout << std::endl << "Logical Device Enabled Extension Count:" << coreExtensions->deviceExtensions.enabled.size() << std::endl;
	for (const auto& extension : coreExtensions->deviceExtensions.enabled) {
		std::cout << "  " << extension << std::endl;
	}

	if (vkCreateDevice(devices.physical, &deviceCreateInfo, nullptr, &this->devices.logical) != VK_SUCCESS) {
		throw std::invalid_argument("failed to create logical device");
	}

	//vkGetDeviceQueue(devices.logical, queue.queueFamilyIndices.graphics, 0, &queue.graphics);
	//vkGetDeviceQueue(devices.logical, queue.queueFamilyIndices.compute, 0, &queue.compute);
	//vkGetDeviceQueue(devices.logical, queue.queueFamilyIndices.present, 0, &queue.present);
	//vkGetDeviceQueue(devices.logical, queue.queueFamilyIndices.transfer, 0, &queue.transfer);

	return devices.logical;

}

VkFormat CoreDevice::chooseSupportedFormat(const std::vector<VkFormat>& formats, VkImageTiling tiling, VkFormatFeatureFlags featureFlags) {
