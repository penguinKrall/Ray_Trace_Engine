...
namespace GVM {
	...
	void Texture::fromglTfImage(tinygltf::Image& gltfimage, TextureSampler textureSampler, CoreBase* coreBase, VkQueue copyQueue)
	{
		...
		vkGetPhysicalDeviceFormatProperties(coreBase->devices.physical, format, &formatProperties);
		...
		vkCreateBuffer(coreBase->devices.logical, &bufferCreateInfo, nullptr, &stagingBuffer);
		vkGetBufferMemoryRequirements(coreBase->devices.logical, stagingBuffer, &memReqs);
		memAllocInfo.allocationSize = memReqs.size;
		memAllocInfo.memoryTypeIndex = coreBase->getMemoryType(memReqs.memoryTypeBits, VK_MEMORY_PROPERTY_HOST_VISIBLE_BIT | VK_MEMORY_PROPERTY_HOST_COHERENT_BIT);
		vkAllocateMemory(coreBase->devices.logical, &memAllocInfo, nullptr, &stagingMemory);
		vkBindBufferMemory(coreBase->devices.logical, stagingBuffer, stagingMemory, 0);

		uint8_t* data;
		vkMapMemory(coreBase->devices.logical, stagingMemory, 0, memReqs.size, 0, (void**)&data);
		...
		vkCreateImage(coreBase->devices.logical, &imageCreateInfo, nullptr, &image);
		vkGetImageMemoryRequirements(coreBase->devices.logical, image, &memReqs);
		memAllocInfo.allocationSize = memReqs.size;
		memAllocInfo.memoryTypeIndex = coreBase->getMemoryType(memReqs.memoryTypeBits, VK_MEMORY_PROPERTY_DEVICE_LOCAL_BIT);
		vkAllocateMemory(coreBase->devices.logical, &memAllocInfo, nullptr, &deviceMemory);
		vkBindImageMemory(coreBase->devices.logical, image, deviceMemory, 0);

		VkCommandBuffer copyCmd = coreBase->objCreate.VKCreateCommandBuffer(VK_COMMAND_BUFFER_LEVEL_PRIMARY, true);
		...
		coreBase->FlushCommandBuffer(copyCmd, copyQueue, coreBase->commandPools.graphics, true);
		...
		VkCommandBuffer blitCmd = coreBase->objCreate.VKCreateCommandBuffer(VK_COMMAND_BUFFER_LEVEL_PRIMARY, true);
		...
		coreBase->FlushCommandBuffer(blitCmd, copyQueue, coreBase->commandPools.graphics, true);
		...
		vkCreateSampler(coreBase->devices.logical, &samplerInfo, nullptr, &sampler);
		...
		vkCreateImageView(coreBase->devices.logical, &viewInfo, nullptr, &view);
	...
	Mesh::Mesh(CoreBase* coreBase, glm::mat4 matrix) {
		...
		coreBase->CreateBuffer2(
			...
			&uniformBlock);
		vkMapMemory(coreBase->devices.logical, uniformBuffer.memory, 0, sizeof(uniformBlock), 0, &uniformBuffer.mapped);
			...
			for (size_t j = 0; j < mesh.primitives.size(); j++) {
				...
				// Vertices
				{
					...
					for (size_t v = 0; v < posAccessor.count; v++) {
						...
						vert.normal = glm::vec4(glm::normalize(glm::vec3(bufferNormals ? glm::make_vec3(&bufferNormals[v * normByteStride]) : glm::vec4(0.0f))), 0.0f);
	...
	void Model::loadFromFile(std::string filename, CoreBase* coreBase, VkQueue transferQueue, uint32_t fileLoadingFlags,float scale)
	{
				...
				if (node->mesh) {
					for (Primitive* primitive : node->mesh->primitives) {
						for (uint32_t i = 0; i < primitive->vertexCount; i++) {
							Vertex& vertex = this->loaderInfo.vertexBuffer[primitive->firstVertex + i];
		...
		coreBase->FlushCommandBuffer(copyCmd, transferQueue, coreBase->commandPools.graphics, true);
