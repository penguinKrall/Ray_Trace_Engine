...
namespace gtp {
	...
	Shader::Shader(EngineCore* coreBase) {
		InitShader(coreBase);
	}

	void Shader::InitShader(EngineCore* coreBase) {
	...
	VkShaderModule Shader::loadShaderModule(const char* fileName, VkDevice device, std::string moduleName) {
		...
		if (file.is_open()) {
			...
			pEngineCore->add([this, &moduleCreateInfo, &shaderModule]()
				{return pEngineCore->objCreate.VKCreateShaderModule(&moduleCreateInfo, nullptr, &shaderModule);},
				moduleName);
	...
	VkPipelineShaderStageCreateInfo Shader::loadShader(std::string fileName, VkShaderStageFlagBits stage, std::string moduleName) {
		...
		shaderStage.module = loadShaderModule(fileName.c_str(), pEngineCore->devices.logical, moduleName);
