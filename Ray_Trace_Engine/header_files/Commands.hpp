...
public:
  ...
  VkCommandPool CreateComputeCommandPool(VkDevice logicalDevice,
                                         uint32_t queueFamilyIndex);
  VkQueue VKCreateQueue(VkDevice logicalDevice, uint32_t queueFamilyIndex,
                        uint32_t queueIndex, VkQueue *queue);

  // -- command buffers
  std::vector<VkCommandBuffer>
  createGraphicsCommandBuffers(VkDevice logicalDevice,
  ...
  //starting to submit again 	if (vkCreateSemaphore(logicalDevice,
  //&semaphoreCreateInfo, nullptr, &sync.presentFinishedSemaphore) !=
  //VK_SUCCESS) { 		throw std::invalid_argument("failed to create imageAvailable
  //semaphore");
  //	}

  //	// Semaphore used to ensure that all commands submitted have been
  //finished before submitting the image to the queue 	if
  //(vkCreateSemaphore(logicalDevice, &semaphoreCreateInfo, nullptr,
  //&sync.renderFinishedSemaphore) != VK_SUCCESS) { 		throw
  //std::invalid_argument("failed to create renderFinished semaphore");
  //	}
  ...
  //command buffer 	fenceCreateInfo.flags = VK_FENCE_CREATE_SIGNALED_BIT;
  //	sync.drawFences.resize(frame_draws);
  //	for (auto& fence : sync.drawFences) {
  //		if (vkCreateFence(logicalDevice, &fenceCreateInfo, nullptr,
  //&fence) != VK_SUCCESS) { 			throw std::invalid_argument("failed to create a
  //fence!");
  //		}
  //	}
  //}
