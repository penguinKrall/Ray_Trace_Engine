...
//    NEED TO BUILD :
//
//    - Using glm (OpenGL Mathematics) include files: need version 0.9.9 or
//    later
//        and GLM_ENABLE_EXPERIMENTAL compile option to enable "quaternion"
//        class
//    - Using include virtualGizmo.h (my virtualGizmoClass) to get imput
//    controls
//          and apply rotations
//    - use c++11 standard
//
//    Use preprocessor define imguiGizmo_INTERPOLATE_NORMALS, to a phong color
//    effect for solid components, but there are more vertex memorized and more
//    operations, even if the visual effect is better... the computational
//    difference is: one normal for vertex  VS  one normal for plain figure
//    (triangle or quad)
//
...
//              for ImGui features, right button drag is active only INSIDE the
//              frame
//      Modifiers:
...
struct imguiGizmo {
  ...
  static void buildPolygon(const vec3 &size, ImVector<vec3> &vtx,
                           ImVector<vec3> &norm);
  ...
  static void buildCone(const float x0, const float x1, const float radius,
                        const int slices);
  static void buildCylinder(const float x0, const float x1, const float radius,
                            const int slices);
  ...
  static void restoreSolidSize() { solidResizeFactor = savedSolidResizeFactor; }
  ...
  static void setDirectionColor(const ImVec4 &color) {
    setDirectionColor(color,
                      ImVec4(color.x, color.y, color.z, STARTING_ALPHA_PLANE));
  }
  ...
  bool getTransforms(quat &q, const char *label, vec3 &dir, float size) {
    ...
    q = angleAxis(acosf(dir.x / len),
                  normalize(vec3(FLT_EPSILON, -dir.z, dir.y)));

    bool ret = drawFunc(label, size);
    if (ret)
      dir = (q * vec3(1.0f, 0.0f, 0.0f)) *
            len; // return vector with original lenght
  ...
  bool getTransforms(quat &q, const char *label, vec4 &axis_angle, float size) {
    ...
    bool ret = drawFunc(label, size);
    if (ret)
      axis_angle = vec4(vec3(axis(q)), angle(q));
  ...
  ///////////////////////////////////////
  static float
      gizmoFeelingRot; // >1 more mouse sensibility, <1 less mouse sensibility
#ifndef IMGUIZMO_USE_ONLY_ROT
...
IMGUI_API bool gizmo3D(const char *, quat &, float = IMGUIZMO_DEF_SIZE,
                       const int = imguiGizmo::mode3Axes |
                                   imguiGizmo::cubeAtOrigin);
IMGUI_API bool gizmo3D(const char *, vec4 &, float = IMGUIZMO_DEF_SIZE,
                       const int = imguiGizmo::mode3Axes |
                                   imguiGizmo::cubeAtOrigin);
IMGUI_API bool gizmo3D(const char *, vec3 &, float = IMGUIZMO_DEF_SIZE,
                       const int = imguiGizmo::modeDirection);

IMGUI_API bool gizmo3D(const char *, quat &, quat &, float = IMGUIZMO_DEF_SIZE,
                       const int = imguiGizmo::modeDual |
                                   imguiGizmo::cubeAtOrigin);
IMGUI_API bool gizmo3D(const char *, quat &, vec4 &, float = IMGUIZMO_DEF_SIZE,
                       const int = imguiGizmo::modeDual |
                                   imguiGizmo::cubeAtOrigin);
IMGUI_API bool gizmo3D(const char *, quat &, vec3 &, float = IMGUIZMO_DEF_SIZE,
                       const int = imguiGizmo::modeDual |
                                   imguiGizmo::cubeAtOrigin);
...
IMGUI_API bool gizmo3D(const char *, vec3 &, quat &, float = IMGUIZMO_DEF_SIZE,
                       const int = imguiGizmo::mode3Axes |
                                   imguiGizmo::cubeAtOrigin);
IMGUI_API bool gizmo3D(const char *, vec3 &, vec4 &, float = IMGUIZMO_DEF_SIZE,
                       const int = imguiGizmo::mode3Axes |
                                   imguiGizmo::cubeAtOrigin);
IMGUI_API bool gizmo3D(const char *, vec3 &, vec3 &, float = IMGUIZMO_DEF_SIZE,
                       const int = imguiGizmo::modeDirection);

IMGUI_API bool
gizmo3D(const char *, vec3 &, quat &, quat &, float = IMGUIZMO_DEF_SIZE,
        const int = imguiGizmo::modeDual | imguiGizmo::cubeAtOrigin);
IMGUI_API bool
gizmo3D(const char *, vec3 &, quat &, vec4 &, float = IMGUIZMO_DEF_SIZE,
        const int = imguiGizmo::modeDual | imguiGizmo::cubeAtOrigin);
IMGUI_API bool
gizmo3D(const char *, vec3 &, quat &, vec3 &, float = IMGUIZMO_DEF_SIZE,
        const int = imguiGizmo::modeDual | imguiGizmo::cubeAtOrigin);
