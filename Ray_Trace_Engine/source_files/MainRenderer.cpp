...
void MainRenderer::LoadParticle(
  ...
  this->accelerationStructures.BuildBottomLevelAccelerationStructure(
      tempParticle->ParticleModel());
...
void MainRenderer::LoadAssets() {
  ...
  this->accelerationStructures.SetTextureOffset(
...
//   this->assets.textureOffset +=
//   static_cast<uint32_t>(pModel->textures.size());
//
...
//       // handle the instances for particles if the model was loaded for use
//       as a
//       // particle
...
//                 transformMatrix[col][row];  // Vulkan expects column-major
//                 order
//           }
//         }
//
...
//         // struct data
//         bottomLevelAccelerationStructureInstance.transform =
//         vkTransformMatrix;
...
//   }
//
//   // if instances 'buffer' is empty, assign a single uninitialized AS
//   instance
...
void MainRenderer::CreateDescriptorSet() {
  ...
  VkAccelerationStructureKHR topLevelAccelerationStructureKHR =
      this->accelerationStructures.GetTopLevelAccelerationStructureKHR();
  ...
  VkDescriptorBufferInfo geometry_nodes_BufferDescriptor =
      this->accelerationStructures.GetGeometryNodesBufferDescriptor();
  ...
  VkDescriptorBufferInfo geometry_nodes_indicesDescriptor =
      this->accelerationStructures.GetGeometryNodesIndexBufferDescriptor();
...
//           // flag that the top level acceleration structure needs to be
//           update updateTLAS = true;
//         }
//       }
//     }
//   }
...
//     // check if model requires bottom level acceleration structure update and
//     is
...
//   // check renderer has been flagged to update top level acceleration
//   structure if (this->updateTLAS) {
...
//       tlasData.accelerationStructureGeometry.geometryType =
//           VK_GEOMETRY_TYPE_INSTANCES_KHR;
//       tlasData.accelerationStructureGeometry.flags =
...
//               &accelerationStructureBuildRangeInfo};
//
//       // build the acceleration structure on the device via a one-time
//       command
...
//   buffers.geometry_nodes_buffer.bufferData.bufferName =
//   "geometry_nodes_buffer";
...
//                                 VK_MEMORY_PROPERTY_HOST_VISIBLE_BIT |
//                                     VK_MEMORY_PROPERTY_HOST_COHERENT_BIT,
//                                 &buffers.geometry_nodes_indices,
//                                 gIndexBufSize, gIndexData) != VK_SUCCESS) {
//     throw std::invalid_argument("failed to create geometry_nodes_index
//     buffer");
//   }
// }
...
//   this->accelerationStructures.RebuildGeometryBuffer(pModel,
//   this->assets.models, this->assets.modelData, this->assets.particle);
//
...
void MainRenderer::HandleAccelerationStructureUpdate() {
...
void MainRenderer::DeleteModel() {
  ...
  this->accelerationStructures.ClearBottomLevelAccelerationStructureInstances();
  ...
  this->accelerationStructures.HandleModelDeleteBottomLevel(
      static_cast<int>(this->assets.defaultTextures.size()),
      this->assets.models);
  ...
  this->accelerationStructures.HandleModelDeleteTopLevel(
      this->assets.models, this->assets.modelData, this->assets.particle);
...
void MainRenderer::HandleLoadModel(gtp::FileLoadingFlags loadingFlags) {
  ...
  // acceleration structure class rebuild geometry buffer
  this->accelerationStructures.RebuildGeometryBuffer(
