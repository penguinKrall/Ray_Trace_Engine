...
public:
  ...
  // map
  VkResult map(VkDevice logicalDevice, VkDeviceSize size = VK_WHOLE_SIZE,
  ...
  VkResult flush(VkDevice logicalDevice, VkDeviceSize size = VK_WHOLE_SIZE,
                 VkDeviceSize offset = 0);

  VkResult invalidate(VkDevice logicalDevice, VkDeviceSize size = VK_WHOLE_SIZE,
                      VkDeviceSize offset = 0);

  void destroy(VkDevice logicalDevice);

  VkBuffer CreateBuffer(VkPhysicalDevice physicalDevice, VkDevice logicalDevice,
                        VkBufferUsageFlags usageFlags, VkDeviceSize bufferSize,
                        std::string bufferName);

  VkDeviceMemory AllocateBufferMemory(VkPhysicalDevice physicalDevice,
