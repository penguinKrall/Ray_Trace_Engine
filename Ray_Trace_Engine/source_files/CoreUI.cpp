...
void CoreUI::InitContext() {
	backends.context = ImGui::CreateContext();
}
...
void CoreUI::InitStyle() {
	...
	imguiStyle.Colors[ImGuiCol_WindowBg] = ImVec4(0.0f, 0.0f, 0.0f, style.bgOpacity);
	...
	imguiStyle.Colors[ImGuiCol_TitleBgCollapsed] = ImVec4(1.00f, 1.00f, 1.00f, 0.51f);
	...
	imguiStyle.Colors[ImGuiCol_ScrollbarGrabHovered] = ImVec4(1.0f, 1.0f, 1.0f, 1.0f);
	imguiStyle.Colors[ImGuiCol_ScrollbarGrabActive] = ImVec4(1.0f, 1.0f, 1.0f, 0.5f);
	...
	imguiStyle.Colors[ImGuiCol_SliderGrabActive] = ImVec4(1.0f, 1.0f, 1.0f, 1.00f);
	...
	imguiStyle.Colors[ImGuiCol_ResizeGripHovered] = ImVec4(0.5f, 0.5f, 0.5f, 0.5f);
	imguiStyle.Colors[ImGuiCol_ResizeGripActive] = ImVec4(1.0f, 1.0f, 1.0f, 1.00f);
	imguiStyle.Colors[ImGuiCol_PlotLines] = ImVec4(0.39f, 0.39f, 0.39f, 1.00f);
	imguiStyle.Colors[ImGuiCol_PlotLinesHovered] = ImVec4(1.00f, 0.43f, 0.35f, 1.00f);
	imguiStyle.Colors[ImGuiCol_PlotHistogram] = ImVec4(0.90f, 0.70f, 0.00f, 1.00f);
	imguiStyle.Colors[ImGuiCol_PlotHistogramHovered] = ImVec4(1.00f, 0.60f, 0.00f, 1.00f);
	imguiStyle.Colors[ImGuiCol_TextSelectedBg] = ImVec4(0.26f, 0.59f, 0.98f, 0.35f);
...
void CoreUI::CreateFontImage() {
	...
	//output to test
	const std::string fontFilename = projDirectory.string() + "/fonts/FiraCode.ttf";

	//add font file
	backends.io->Fonts->AddFontFromFileTTF(fontFilename.c_str(), 16.0f * properties.scale);

	//get texture data as rgba32
	backends.io->Fonts->GetTexDataAsRGBA32(&fontData, &fontImage.texWidth, &fontImage.texHeight);
	...
	targetImageCreateInfo.extent.width = static_cast<uint32_t>(fontImage.texWidth);
	targetImageCreateInfo.extent.height = static_cast<uint32_t>(fontImage.texHeight);
	...
	targetImageCreateInfo.usage = VK_IMAGE_USAGE_SAMPLED_BIT | VK_IMAGE_USAGE_TRANSFER_DST_BIT;
	...
	fontImageViewCreateInfo.subresourceRange.aspectMask = VK_IMAGE_ASPECT_COLOR_BIT;
...
void CoreUI::CreateUIRenderPass() {
	...
	subpassDescription.colorAttachmentCount = static_cast<uint32_t>(swapchainImageReference.size());
...
void CoreUI::CreateUIPipeline() {
	...
	std::cout << "Project Directory: " << projDirectory.string() + "/shaders/compiled/UIOverlay.vert.spv" << std::endl;

	//vertex
	shaderStages.push_back(shader.loadShader(projDirectory.string() + "/shaders/compiled/UIOverlay.vert.spv",
		VK_SHADER_STAGE_VERTEX_BIT, "UIVertexShaderModule"));

	//fragment
	shaderStages.push_back(shader.loadShader(projDirectory.string() + "/shaders/compiled/UIOverlay.frag.spv",
...
void CoreUI::CreateResources() {
	...
	//upload size
	VkDeviceSize uploadSize = fontImage.texWidth * fontImage.texHeight * 4 * sizeof(char);
	...
	bufferImageCopyData.imageOffset = { 0, 0, 0, };
	bufferImageCopyData.imageExtent = { static_cast<uint32_t>(fontImage.texWidth), static_cast<uint32_t>(fontImage.texHeight), 1 };

	//copy buffer to image
	vkCmdCopyBufferToImage(cmdBuffer, stagingBuffer.bufferData.buffer, fontImage.image,
...
void CoreUI::update(int currentFrame) {
	...
	if (!backends.drawData) { return; }

	//vertex buffer size
	VkDeviceSize vertexBufferSize =
		(static_cast<long long>(backends.drawData->TotalVtxCount + backends.drawData->TotalVtxCount * 2)) * (sizeof(ImDrawVert) * 2);

	//index buffer size
	VkDeviceSize indexBufferSize =
		(static_cast<long long>(backends.drawData->TotalIdxCount + backends.drawData->TotalIdxCount * 2)) * (sizeof(ImDrawIdx) * 2);
	...
	if ((buffers.vertex[currentFrame].bufferData.buffer == VK_NULL_HANDLE) || (buffers.vertexCount != backends.drawData->TotalVtxCount)) {
	...
	//update index buffer
	if ((buffers.index[currentFrame].bufferData.buffer == VK_NULL_HANDLE) || (buffers.indexCount < backends.drawData->TotalIdxCount)) {
	...
	//upload data
	ImDrawVert* vtxDst = (ImDrawVert*)buffers.vertex[currentFrame].bufferData.mapped;
	ImDrawIdx* idxDst = (ImDrawIdx*)buffers.index[currentFrame].bufferData.mapped;
...
void CoreUI::Input() {
	...
	ImGui::Begin("topFrameBar", 0, ImGuiWindowFlags_MenuBar | ImGuiWindowFlags_NoBackground |
	...
	if (ImGui::BeginMenuBar()) {
		...
		//ImGui::SameLine(ImGui::GetWindowWidth() - ImGui::CalcTextSize(" framerate: 000.000 ms/frame ( 0.0 FPS )  ").x);
...
void CoreUI::DrawUI(const VkCommandBuffer commandBuffer, int currentFrame) {
	...
	VkRenderPassBeginInfo UIPassBeginInfo = { VK_STRUCTURE_TYPE_RENDER_PASS_BEGIN_INFO, nullptr,
	renderData.renderPass, renderData.framebuffer[currentFrame],
	renderArea,  static_cast<uint32_t>(clearValue.size()),
	clearValue.data() };

	//begin render pass
	vkCmdBeginRenderPass(commandBuffer,
		&UIPassBeginInfo, VK_SUBPASS_CONTENTS_INLINE);

	//bind pipeline
	vkCmdBindPipeline(commandBuffer, VK_PIPELINE_BIND_POINT_GRAPHICS, renderData.pipeline);
	
	//bind descriptor sets
	vkCmdBindDescriptorSets(commandBuffer, VK_PIPELINE_BIND_POINT_GRAPHICS,
		renderData.pipelineLayout, 0, 1, &descriptor.descriptorSet, 0, NULL);

	//transform
	pushConstant.scale = glm::vec2(2.0f / backends.io->DisplaySize.x, 2.0f / backends.io->DisplaySize.y);
	pushConstant.translate = glm::vec2(-1.0f);

	//bind push constants
	vkCmdPushConstants(commandBuffer, renderData.pipelineLayout,
		VK_SHADER_STAGE_VERTEX_BIT, 0, sizeof(UIPushConstant), &pushConstant);

	//buffer offsets
	std::vector<VkDeviceSize> offsets = { 0 };

	//bind vertex and index buffers
	vkCmdBindVertexBuffers(commandBuffer, 0, 1, &buffers.vertex[currentFrame].bufferData.buffer, offsets.data());
	vkCmdBindIndexBuffer(commandBuffer, buffers.index[currentFrame].bufferData.buffer, 0, VK_INDEX_TYPE_UINT16);
