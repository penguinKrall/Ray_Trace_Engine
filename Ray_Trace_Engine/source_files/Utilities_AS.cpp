...
uint64_t Utilities_AS::getBufferDeviceAddress(EngineCore* pEngineCore, VkBuffer buffer) {
	VkBufferDeviceAddressInfoKHR bufferDeviceAI{};
	bufferDeviceAI.sType = VK_STRUCTURE_TYPE_BUFFER_DEVICE_ADDRESS_INFO;
	bufferDeviceAI.buffer = buffer;
	return pEngineCore->coreExtensions->vkGetBufferDeviceAddressKHR(pEngineCore->devices.logical, &bufferDeviceAI);
}

void Utilities_AS::createScratchBuffer(EngineCore* pEngineCore, gtp::Buffer* buffer, VkDeviceSize size, std::string name) {
	//create buffer
	pEngineCore->add([pEngineCore, buffer, size]() {return buffer->CreateBuffer(pEngineCore->devices.physical,
		pEngineCore->devices.logical, VK_BUFFER_USAGE_STORAGE_BUFFER_BIT |
	...
	//allocate memory
	pEngineCore->add([pEngineCore, buffer, name]() {return buffer->AllocateBufferMemory(pEngineCore->devices.physical,
		pEngineCore->devices.logical, "scratchBufferMemory_" + name);}, "scratchBufferMemory_" + name);

	//bind memory
	if (vkBindBufferMemory(pEngineCore->devices.logical, buffer->bufferData.buffer, buffer->bufferData.memory, 0)) {
		throw std::invalid_argument("Failed to bind " + name + " scratch buffer memory");
	}

	buffer->bufferData.bufferDeviceAddress.deviceAddress = Utilities_AS::getBufferDeviceAddress(pEngineCore, buffer->bufferData.buffer);

}

void Utilities_AS::createAccelerationStructureBuffer(EngineCore* coreBase, VkDeviceMemory* memory, VkBuffer* buffer,
...
void Utilities_AS::createStorageImage(EngineCore* pEngineCore, StorageImage* storageImage, std::string name) {

	//image create info
	VkImageCreateInfo imageCreateInfo{};
	imageCreateInfo.sType = VK_STRUCTURE_TYPE_IMAGE_CREATE_INFO;
	imageCreateInfo.imageType = VK_IMAGE_TYPE_2D;
	imageCreateInfo.format = pEngineCore->swapchainData.assignedSwapchainImageFormat.format;
	imageCreateInfo.extent.width = pEngineCore->swapchainData.swapchainExtent2D.width;
	imageCreateInfo.extent.height = pEngineCore->swapchainData.swapchainExtent2D.height;
	...
	//create image
	pEngineCore->add([pEngineCore, &imageCreateInfo, &storageImage]()
		{return pEngineCore->objCreate.VKCreateImage(&imageCreateInfo, nullptr, &storageImage->image);}, name);

	//image memory requirements
	VkMemoryRequirements memReqs;
	vkGetImageMemoryRequirements(pEngineCore->devices.logical, storageImage->image, &memReqs);
	...
	pEngineCore->add([pEngineCore, &memoryAllocateInfo, &storageImage]()
		{return pEngineCore->objCreate.VKAllocateMemory(&memoryAllocateInfo, nullptr, &storageImage->memory);},
		name + "_memory");

	//bind image memory
	if (vkBindImageMemory(pEngineCore->devices.logical, storageImage->image, storageImage->memory, 0) != VK_SUCCESS) {
	...
	imageViewCreateInfo.format = pEngineCore->swapchainData.assignedSwapchainImageFormat.format;
	...
	pEngineCore->add([pEngineCore, &imageViewCreateInfo, &storageImage]()
		{return pEngineCore->objCreate.VKCreateImageView(&imageViewCreateInfo, nullptr, &storageImage->view);},
		name + "view");
	...
	gtp::Utilities_EngCore::setImageLayout(cmdBuffer, storageImage->image,
...
void Utilities_AS::createBLAS(EngineCore* pEngineCore,
				...
				if (primitive->indexCount > 0) {
					...
					vertexBufferDeviceAddress.deviceAddress =
						Utilities_AS::getBufferDeviceAddress(pEngineCore,
							model->vertices.buffer);// +primitive.firstVertex * sizeof(gtp::Vertex);
	...
	Utilities_AS::createAccelerationStructureBuffer(pEngineCore, &BLAS->memory, &BLAS->buffer,
		&blasData->accelerationStructureBuildSizesInfo, "Utilities_AS::createBLAS___AccelerationStructureBuffer");
	...
	pEngineCore->add([pEngineCore, &BLAS, &accelerationStructureCreateInfo]()
		{return pEngineCore->objCreate.VKCreateAccelerationStructureKHR(&accelerationStructureCreateInfo, nullptr,
			&BLAS->accelerationStructureKHR);}, "Utilities_AS::createBLAS___BLASAccelerationStructureKHR");

	//create scratch buffer for acceleration structure build
	//gtp::Buffer scratchBuffer;
	Utilities_AS::createScratchBuffer(
		pEngineCore, &BLAS->scratchBuffer, blasData->accelerationStructureBuildSizesInfo.buildScratchSize, "Utilities_AS::createBLAS___blas_ScratchBufferBLAS");
	...
	BLAS->deviceAddress =
		pEngineCore->coreExtensions->vkGetAccelerationStructureDeviceAddressKHR(pEngineCore->devices.logical,
			&accelerationDeviceAddressInfo);
}

std::vector<gtp::Model::Vertex> Utilities_AS::GetVerticesFromBuffer(VkDevice device, gtp::Model* model) {

		void* data = nullptr;

		VkDeviceSize bufferSize = static_cast<uint32_t>(model->vertexCount) * sizeof(gtp::Model::Vertex);
